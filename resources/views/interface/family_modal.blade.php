    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">         
                <form action="{{url('donation/families/'.$family['id'])}}" method="post" class="loginV__form">
                    {{ csrf_field() }}
                    {{ Form::hidden('pay_type','knet',['id'=>'pay_type']) }}

                    <h3 class="loginModal_title">{{trans('admin.donate')}} : {{$family[$lang.'_name']}}</h3>
                    <p class="logintModal_des">{{number_format($family['amount'] / $currency_info['equal'], $currency_info['currency_format'], '.', '') . ' ' . $currency_info[$lang . '_currency']}}</p>
                    <div class="form__row">
                        @if(!Auth::Check())
...
<div class="modal loginModal fade" id="giftModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <img src="{{url('interface')}}/img/si.png" alt="" class="check__thumb">
            <div class="modal-body">         
                <form action="{{url('gift/families/'.$family['id'])}}" method="post" class="loginV__form">
                    {{ csrf_field() }}
                    {{ Form::hidden('gift_id',null,['id'=>'gift_id']) }}

                    <h3 class="loginModal_title">{{trans('admin.family_gift')}}</h3>
                    <p class="logintModal_des">{{$family[$lang.'_name']}}</p>
                    <div class="form__row">
                        <div class="col-12 col-lg-6">
                    ...
                    <button class="login__btn" type='submiut'>{{trans('admin.family_gift')}}</button>
                </form>
            </div>
        </div>
    </div>
