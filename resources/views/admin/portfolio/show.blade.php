...
<section class="content-header">
    <h1>
        {{trans('admin.portfolio')}} : {{$company[$lang.'_name']}}
    </h1>
    <ol class="breadcrumb">
        ...
        <div class="col-12">
            ...
            <div class="box">
                <div class="box-header">
                    <a href="{{URL::to('admin/portfolio/create')}}" class="btn btn-lg bg-black">{{trans('admin.portfolio_add')}}</a>
                ...
                <div class="box-body">
                    <table id="example" class="table table-bordered table-hover display nowrap margin-top-10 table-responsive">
                        ...
                        <tbody>
                            @foreach($company['Portfolio'] as $one)
                            <tr>
                                <td style="vertical-align: middle;">{{$one['User'][$lang.'_name']}}</td>
                                <td style="vertical-align: middle;">{{$one[$lang.'_name']}}</td>
                                <td style="vertical-align: middle;">
                                    @if($one['active'] != 'yet')
                                    <label class="switch" style="margin-bottom: 0;">
                                        @if($one['active'] == 'yes')
                                        <input class="switch_active" page='portfolio' id="{{$one['id']}}" type="checkbox" checked="">
                                        @else
                                        <input class="switch_active" page='portfolio' id="{{$one['id']}}" type="checkbox">
                                        @endif
                                ...
                                <td style="vertical-align: middle;">
                                    {{ Form::open(array('url' =>'admin/portfolio/'.$one->id.'/edit', 'method' => 'GET')) }}
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    {{ Form::open(array('url' =>'admin/portfolio/'.$one->id, 'method' => 'DELETE')) }}
...
<div class="modal fade" id="Modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        {{ Form::open(array('url' =>'admin/delete_all/portfolio', 'method' => 'POST')) }}
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <select id="ids" name="ids[]" class="form-control select2" multiple="multiple" data-placeholder="{{trans('admin.delete_all')}}" style="width: 100%;" required="">
                    @foreach($company['Portfolio'] as $one)
                    <option value="{{$one['id']}}">{{$one['User'][$lang.'_name'].' - '.$one[$lang.'_name']}}</option>
                    @endforeach
                </select>
            </div>
            <div class="modal-footer">
