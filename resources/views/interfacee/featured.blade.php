...
<section class="page-header">
    <div class="container">
        <h2 class="mb-0">{{trans('admin.featured_products')}}</h2>
    </div>
</section>
            ...
            <div class="col-md-3 col-5">
                <div class="sorting">
                    ...
                    <div class="custom-control custom-radio">
                        ...
                        <label class="custom-control-label" for="z_a">{{trans('admin.a_z')}}</label>
                    </div>
                    <div class="custom-control custom-radio">
...
<script>
    $(window).on('scroll', function () {
        if ($(window).scrollTop() >= $('#products').offset().top + $('#products').outerHeight() - window.innerHeight - 100) {
            ...
            if (page <= last_page) {
                ...
                $.ajax({
                    type: "GET",
                    url: link + "/featured?page=" + page + "&sort=" + sort + "&json=yes",
    ...
    $('input[name=sort]').change(function () {
        ...
        $.ajax({
            type: "GET",
            url: link + "/featured?page=1&sort=" + sort + "&json=yes",
            success: function (data) {
                $('#products').html(data);
            }
        });
